...
module.exports = function (logger) {
    ...
    var insert = {
        summoner: async function (summoner) {
            return await runQuery('INSERT INTO summoner SET ? ON DUPLICATE KEY UPDATE name = VALUES(name), profileIconId = VALUES(profileIconId), revisionDate = VALUES(revisionDate), summonerLevel = VALUES(summonerLevel), lastQueried = VALUES(lastQueried)', summoner);
        },
        champions: async function (champion) {
            return await runQuery('INSERT INTO champions SET ? ON DUPLICATE KEY UPDATE title = VALUES(title)', champion);
        },
        ...
        runes: async function (rune) {
            return await runQuery('INSERT INTO runes SET ? ON DUPLICATE KEY UPDATE shortDesc = VALUES(shortDesc), longDesc = VALUES(longDesc), `key` = VALUES(`key`), name = VALUES(name), icon = VALUES(icon), runePathId = VALUES(runePathId), runePathName = VALUES(runePathName)', rune);
        },
        summonerSpells: async function (spell) {
            return await runQuery('INSERT INTO summonerSpells SET ? ON DUPLICATE KEY UPDATE `key` = VALUES(`key`), name = VALUES(name), description = VALUES(description), summonerLevel = VALUES(summonerLevel)', spell);
        },
        championMasteries: async function (mastery) {
            return await runQuery('INSERT INTO championMasteries SET ? ON DUPLICATE KEY UPDATE championLevel = VALUES(championLevel), championPoints = VALUES(championPoints), lastPlayTime = VALUES(lastPlayTime), championPointsSinceLastLevel = VALUES(championPointsSinceLastLevel), championPointsUntilNextLevel = VALUES(championPointsUntilNextLevel), chestGranted = VALUES(chestGranted), tokensEarned = VALUES(tokensEarned)', mastery);
        },
        leaguePosition: async function (league) {
            return await runQuery('INSERT INTO leaguePosition SET ? ON DUPLICATE KEY UPDATE leagueId = VALUES(leagueId), leagueName = VALUES(leagueName), tier = VALUES(tier), rank = VALUES(rank), playerOrTeamName = VALUES(playerOrTeamName), leaguePoints = VALUES(leaguePoints), wins = VALUES(wins), losses = VALUES(losses), veteran = VALUES(veteran), inactive = VALUES(inactive), freshBlood = VALUES(freshBlood), hotStreak = VALUES(hotStreak)', league);
        },
        matchList: async function (match) {
            await runQuery('INSERT INTO matches (id) VALUES (?) ON DUPLICATE KEY UPDATE id = VALUES(id)', match.gameId);
    ...
    var select = {
        newGames: async function (limit) {
            return await runQuery('SELECT id FROM matches WHERE gameCreation IS NULL ORDER BY id DESC LIMIT ?', [limit]);
        },
